...
Over gRPC an ABORTED status maps to `PartialUploadException` only when its message starts with the daemon's fixed `Partial upload:` prefix (anchored, as in antd-rust); any other ABORTED stays a `ForkException`. `retentionKnown` is true when the message starts with the count layout (`Partial upload: S/T chunks stored, F failed …`) and all three counts convert to a `Long`; the daemon's "paid attempt retained" hint then decides `retryable`. If the layout does not match or any count fails to convert (e.g. one past `Long.MAX_VALUE`), all three counts read as zero and both flags as false, even if the hint is present.
