...
public:
    ...
    /// are parsed from the status message, `retryable` only when the counts
    /// parse, and any other ABORTED stays a plain AntdError). The stored
    ...
    /// some sub-batches unpaid is reported with `retryable == false`:
    /// re-prepare the same content to pay for and store only the remainder.
    /// See docs/external-signer-flow.md section 6.
