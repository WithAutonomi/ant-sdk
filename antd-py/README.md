...
- **gRPC.** The counts gate both flags. `retention_known` is `True` only when the message matches `Partial upload: S/T chunks stored, F failed` and all three counts fit in a u64; the `paid attempt retained` hint then decides `retryable`. Otherwise the counts are `0` and both flags are `False`.
