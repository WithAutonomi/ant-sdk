...
Over REST the counts and flag come from the structured error body. Over gRPC a partial upload is an `ABORTED` status whose message **starts with** the daemon's fixed `Partial upload:` prefix, and the counts and flag are parsed from that message. An `ABORTED` that does not start with the prefix (including one that only embeds it, e.g. `upstream error: Partial upload: ...`) is not a partial upload and raises `ForkError` as before. Full contract: `docs/external-signer-flow.md` §6.
...
- **gRPC.** The counts gate the retry. `retryable` is `True` only when the message matches `Partial upload: S/T chunks stored, F failed`, all three counts fit in a u64, **and** the `paid attempt retained` hint is present. Otherwise the counts are `0` and `retryable` is `False`.
