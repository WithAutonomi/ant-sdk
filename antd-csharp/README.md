...
`PartialUploadException` extends `NetworkException` because a partial upload has always arrived as a 502, so existing `catch (NetworkException)` blocks keep matching; catch the derived type first to branch on the counts. Over REST the fields come from the structured error body; over gRPC an `ABORTED` status maps to `PartialUploadException` only when its status detail starts with the daemon's fixed `Partial upload:` prefix, with the counts and `Retryable` parsed from the rest of the detail (counts that fail to parse read as zero; `Retryable` follows the `paid attempt retained` hint). Any other `ABORTED`, including one that quotes `Partial upload:` further into its detail, keeps the `ForkException` mapping.
