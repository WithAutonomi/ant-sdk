...
`Retryable` implies `RetentionKnown`. `PartialUploadException` extends `NetworkException` because a partial upload has always arrived as a 502, so existing `catch (NetworkException)` blocks keep matching; catch the derived type first to branch on the counts. Over REST the counts come from the structured error body, and `RetentionKnown` is `true` only when `retryable` is a JSON boolean (missing, `null`, `"true"` or `1` read as unknown). Over gRPC an `ABORTED` status maps to `PartialUploadException` only when its status detail starts with the daemon's fixed `Partial upload:` prefix; `RetentionKnown` is `true` only when all three counts in the detail parse, and the `paid attempt retained` hint then decides `Retryable`. If the counts do not match the daemon's layout or do not convert (an overflow, non-ASCII digits), all three read as zero and both flags are `false`, even with the hint. Any other `ABORTED`, including one that quotes `Partial upload:` further into its detail, keeps the `ForkException` mapping.
